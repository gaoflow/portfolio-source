...
summary: 'A first-month toolkit computing Reynolds, Mach, Prandtl, Nusselt, Grashof and Rayleigh under strict SI dimensional guards, with a property table anchored to a cited textbook source.'
methodLine: 'Defining equations · SI dimension-vector guards · anchored property table'
role: 'Solo study project'
duration: 'First month in mechanics'
heroMetrics:
  - { label: 'Reference Re exactness', value: '0.0 error' }
  - { label: 'Ra = Gr·Pr identity', value: '4.4e-16' }
  - { label: 'Invalid calls rejected', value: '10/10' }
keyOutputs:
  - 'Six dimensionless groups with dimension guards that reject inconsistent units (e.g. kinematic for dynamic viscosity) instead of computing silently.'
  - 'Air/water property table exact at five anchors transcribed from Incropera 7th ed., with a NIST WebBook cross-check URL.'
  - 'CLI with JSON mode; every published number regenerated by a gate-runner that exits nonzero on failure.'
featured: false
order: 11
studySequence: 1
