...
The verification plan failed on first contact. The plan was to run the grid-convergence study against the closed-form solution. At $N=160$ the maximum nodal error came back $1.34\times10^{-11}$ K — roundoff, not truncation error. Central differences are exact for quadratics, the exact solution is quadratic, and the half-cell Robin balance is exact for quadratics too: the face-flux truncation term $-(h/2)\,kT''$ cancels the half-cell generation term $(h/2)\,q'''$ exactly. Refining the grid further would have measured floating-point noise.
