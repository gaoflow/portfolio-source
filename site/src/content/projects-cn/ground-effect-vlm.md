...
当我们看 F1 时，总会听到“地面效应”。翼面越靠近地面，下压力往往越强。但地面明明只是一条平路，它为什么会改变上方翼面的受力？离得太近以后，结果为什么又不再可信？我做这个项目，就是想先把这两个问题想明白。

真车上的底板、轮胎、扩散器和缝隙泄漏会同时影响气流。把整辆车直接放进模型，反而很难看清地面本身做了什么。所以我用镜像涡原理写了一个低阶涡格法（VLM）工具，先研究一张简单的矩形翼。

2022 至 2025 年的 F1 赛车[使用成形的 Venturi 地板隧道加强地面效应](https://www.formula1.com/en/latest/article/10-things-you-need-to-know-about-the-all-new-2022-f1-car.4OLg8DrXyzHzdoGrbqp6ye)。下面这张整车图只用来交代当时的规则背景，没有参与计算。
...
想象一张薄薄的长方形翼，以固定角度迎着气流。开始时它离地很远，接着一点点下降，其他条件都不变。我要观察三件事：载荷沿翼展怎样变化，总升力增加多少，以及产生同样升力时需要付出多少诱导阻力。

我没有导入 CAD，也没有照着某一代 F1 赛车建地板。代码里的翼没有厚度、后掠和扭转。弦长取 $c=1$，翼展取 $b=4c$，迎角固定为 $4^\circ$。这个简化让我只改变离地高度，不把轮胎、扩散器和车身干扰带进来。

后文统一使用航空里常见的“升力”记法。把翼翻转以后，力的方向变成 F1 所说的下压力；这个项目关注的是地面怎样改变载荷，力朝上还是朝下不影响这里讨论的近地机制。

这个模型只用来观察趋势。它没有黏性和流动分离，因此不能预测真实赛车会在哪个车高失速。

## 涡是什么，又是怎样产生的

大白话说，涡就是一团绕着某个中心打转的空气。空气可以一边向后流，一边绕圈。烟雾、云气和水汽能把旋转显示出来，但烟本身不是涡，它只是跟着空气运动。
...
页首的 FAA 图画出了两条尾涡，以及它们共同造成的下洗。为了让形成过程更直观，我用 AI 做了下面这个原理动画。动画先画压力差怎样推动翼尖绕流，再画绕流怎样卷成尾涡。它不是 CFD 结果。

<iframe class="article-demo" src="/labs/ground-effect-vortex/" title="翼尖涡形成动画" loading="lazy"></iframe>

## VLM 怎样表示一片翼

真实尾流不会自动长成几条笔直的线。VLM 为了降低计算量，用马蹄涡近似一小段翼面产生的升力和下洗。马蹄涡中间横着的一段叫束缚涡，后面两条长线表示尾涡。

我沿翼展把矩形翼均匀切成 64 段，每段放一个马蹄涡。束缚涡位于四分之一弦线，检查气流是否穿过翼面的控制点位于四分之三弦线，尾涡向后延伸 80 个弦长。
...
下面的 MIT 图用一个二维涡说明了同样的关系。地面上方的 $\Gamma$ 是真实涡，地面下方的 $-\Gamma$ 是镜像涡，两者到地面的距离都是 $b$。
...
下面的高度扫描图回答两个问题。左图看固定迎角下的升力相对自由空间增加多少，右图看单位升力对应的诱导阻力代价怎样变化。横轴都是 $h/c$，数值越小，翼越靠近地面。
...
第三个数降得很多，主要是因为分母 $C_L^2$ 变大了，并不代表绝对阻力下降。这个模型支持的结论是：得到同样多的升力时，诱导阻力代价更低。它不支持“翼一靠近地面，绝对阻力就会下降”。

## 我怎样确认结果没有算歪

得到趋势以后，我没有马上把它当成结论。我先检查镜像涡是否真的挡住了地面，再检查翼离地很远时能否回到自由空间，最后检查理论量级、面元细化和左右对称性。
...
升力线斜率仍有 11.01% 的差异。我没有用校准把它抹掉。面元从 64 增加到 96 时，升力只变化 0.258%，说明继续加密面元也消不掉这部分差距。它主要来自模型本身的简化。
...
| 用途 | 是否适合 | 原因 |
|---|---|---|
| 检查地面效应的方向和数量级 | 适合 | 单独保留了镜像机制 |
| 比较有限范围内的离地高度 | 适合 | 14 个状态使用同一求解器 |
| 为更复杂的模型提供低成本参考 | 适合 | 可以比较趋势和远场恢复 |
| 比较赛车地板几何 | 不适合 | 没有地板、扩散器、轮胎和泄漏 |
| 预测分离或失速 | 不适合 | 模型无黏且线性 |

这套模型是稳态、不可压缩、无黏和零厚度的。它没有道路边界层、车身阻塞、压力恢复和流动分离。我只把它当成检查方向、数量级和数值流程的低成本工具。

真实赛车研究还需要加入有限厚度、弦向载荷、运动地面边界层、车高和俯仰变化、轮胎、泄漏、扩散器、网格敏感性和流动分离。

## 这次学习让我记住的两件事

第一，低阶模型也能告诉我应该在哪里停止外推。第二，固定迎角升力、绝对诱导阻力和单位升力代价必须分开报告，因为它们回答的是不同问题。
